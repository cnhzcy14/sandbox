...
public:
    ...
    int process_frame_with_open_cl(cv::Mat& frame);
    int process_cl_buffer_with_opencv(cv::Mat& frame);
    int process_cl_image_with_opencv();
    int process_image_with_opencv();
...
private:
    ...
    cv::Ptr<cv::FastFeatureDetector> feature_detector_ptr_;
    cv::Ptr<cv::FastFeatureDetector> feature_detector_ptr_2_;
    std::vector<cv::KeyPoint>   features_;
    std::vector<cv::KeyPoint>   features_2_;
};
...
int App::initOpenCL()
{
    ...
    for (i = 0; i < m_platform_ids.size(); i++)
    {
        ...
        size_t psize;
        cv::AutoBuffer<char> buf;
        res = clGetPlatformInfo(m_platform_ids[i], CL_PLATFORM_NAME, 0, 0, &psize);
        if (CL_SUCCESS != res)
            return -1;

        buf.resize(psize);
        res = clGetPlatformInfo(m_platform_ids[i], CL_PLATFORM_NAME, psize, buf, 0);
        if (CL_SUCCESS != res)
            return -1;
        ...
        cv::ocl::attachContext(name, m_platform_ids[i], m_context, m_device_id);

        break;
    }

    feature_detector_ptr_ = cv::FastFeatureDetector::create(30, false);
    feature_detector_ptr_2_ = cv::FastFeatureDetector::create(30, false);
...
int App::process_frame_with_open_cl(cv::Mat& frame)
{
    cl_int res = CL_SUCCESS;
    CV_Assert(&m_mem_obj);
    cl_kernel kernel = 0;

    if (0 == m_mem_obj || 0 == m_img_src)
    {
...
int App::process_cl_buffer_with_opencv(cv::Mat& frame)
{
    ...
    features_.clear();
    feature_detector_ptr_->detect(m_uframe, features_);
...
// to OpenCV UMat and then do blur on these data
int App::process_cl_image_with_opencv()
{
    // process right half of frame in OpenCV
    // cv::Point pt(u.cols / 2, 0);
    // cv::Size  sz(u.cols / 2, u.rows);
    // cv::Rect roi(pt, sz);
    // cv::UMat uroi(u, roi);
    cv::blur(m_uframe, m_uframe, cv::Size(3, 3), cv::Point(-3, -3));
...
int App::process_image_with_opencv()
{
    // frame.copyTo(u);
    ...
    // cv::Rect roi(pt, sz);
    // cv::Mat uroi(u, roi);
    // cv::blur(m_frameGray, m_frameGray, cv::Size(7, 7), cv::Point(-3, -3));
    features_.clear();
    feature_detector_ptr_->detect(m_frameGray, features_);
    return 0;
}
...
int App::run()
{
    ...
    while (isRunning() && nextFrame(m_frame))
    {
        ...
        {   
            ...
            process_frame_with_open_cl(m_frameGray);
            if (useBuffer())
            {
                cv::ocl::convertFromBuffer(m_mem_obj, m_frameGray.step[0], m_frameGray.rows, m_frameGray.cols, m_frameGray.type(), m_uframe);
                // cv::ocl::setUseOpenCL(false);
                auto t1 = std::chrono::high_resolution_clock::now();
                // cv::blur(m_uframe, m_uframe, cv::Size(3, 3), cv::Point(-3, -3));
                
                // process_cl_buffer_with_opencv(m_frameGray);
                features_.clear();
                feature_detector_ptr_->detect(m_uframe, features_);
                auto t2 = std::chrono::high_resolution_clock::now();
                features_2_.clear();
                feature_detector_ptr_2_->detect(m_uframe, features_2_);
                auto t3 = std::chrono::high_resolution_clock::now();

                p0 = 1.e-3 * std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
                p1 = 1.e-3 * std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
                p2 = 1.e-3 * std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count();
                if (m_mem_obj)
                    clReleaseMemObject(m_mem_obj);
                m_mem_obj = 0;
            }
            ...
            {
                cv::ocl::convertFromImage(m_mem_obj, m_uframe);
                auto t2 = std::chrono::high_resolution_clock::now();
                process_cl_image_with_opencv();
                auto t3 = std::chrono::high_resolution_clock::now();
                // p1 = 1.e-3 * std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
                // p2 = 1.e-3 * std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count();
            }
            VLOG(1) << std::fixed << std::setprecision(2) << p0 << "," << p1 << "," << p2 << "," << p0+p1+p2;
            m_uframe.copyTo(img_to_show); 
            // cv::drawKeypoints(m_uframe, features_, img_to_show);
        }
        else
        {
            auto t0 = std::chrono::high_resolution_clock::now();
            ...
            auto t1 = std::chrono::high_resolution_clock::now();
            double p0 = 1.e-3 * std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
            VLOG(1) << std::fixed << std::setprecision(2) << p0;
            m_frameGray.copyTo(img_to_show);
            // cv::drawKeypoints(m_frameGray, features_, img_to_show);
        }
