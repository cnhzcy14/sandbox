...
int App::initOpenCL()
{
    ...
    for (i = 0; i < m_platform_ids.size(); i++)
    {
        ...
        char *kernelSrc = getKernelSource("/home/cnhzcy14/work/project/sandbox/opencv/test/test_kernels.cl");
        ...
        m_kernelImg = clCreateKernel(m_program, "bitwise_inv_img_8uC1_ip", &res);
...
int App::process_frame_with_open_cl(cv::Mat &frame, bool use_buffer, cl_mem *mem_obj)
{
    ...
    if (0 == mem)
    {
        ...
        {
            cl_mem_flags flags_src = CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR;
            ...
            desc_src.buffer = 0;
            mem = clCreateImage(m_context, flags_src, &fmt, &desc_src, frame.ptr(), &res);
            if (0 == mem || CL_SUCCESS != res)
                return -1;
    ...
    if (use_buffer)
    {
        ...
        res |= clEnqueueNDRangeKernel(m_queue, m_kernelBuf, 2, 0, size, 0, 0, 0, &asyncEvent);
        if (CL_SUCCESS != res)
            return -1;
    }
    else
    {
        ...
        res |= clEnqueueNDRangeKernel(m_queue, m_kernelImg, 2, 0, size, 0, 0, 0, &asyncEvent);
        if (CL_SUCCESS != res)
            return -1;
    }
