...
public:
    ...
    int process_with_opencv(cv::Mat& u);
...
private:
    ...
    cv::Ptr<cv::FastFeatureDetector> m_feature_detector_ptr;
    // cv::Ptr< cv::ORB >               m_feature_detector_ptr;
...
App::App(CommandLineParser& cmd)
{
    ...
    m_feature_detector_ptr = cv::FastFeatureDetector::create(30, true);
    // m_feature_detector_ptr = cv::ORB::create(192, 1.2f, 1, 2, 0, 1, ORB::FAST_SCORE, 2, 20);
...
int App::process_cl_buffer_with_opencv(cl_mem buffer, size_t step, int rows, int cols, int type, cv::UMat& u)
{
    ...
    m_features.clear();
    m_feature_detector_ptr->detect(u, m_features);
...
int App::process_with_opencv(cv::Mat& u)
{
    m_features.clear();
    m_feature_detector_ptr->detect(u, m_features);
...
int App::run()
{
    ...
    while (isRunning() )
    {
        ...
        else
        {
            ...
            process_with_opencv(m_frameGray);
        }

        timerEnd();

        drawKeypoints( m_frame, m_features, img_to_show );

        putText(img_to_show, "Version : " + m_platformInfo.Version(), Point(5, 30), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 100, 0), 2);
        // putText(img_to_show, "Name : " + m_platformInfo.Name(), Point(5, 60), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 100, 0), 2);
        putText(img_to_show, "Number : " + to_string(m_features.size()), Point(5, 60), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 100, 0), 2);
        putText(img_to_show, "Device : " + m_deviceInfo.Name(), Point(5, 90), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 100, 0), 2);
        cv::String memtype = useBuffer() ? "buffer" : "image";
        putText(img_to_show, "interop with OpenCL " + memtype, Point(5, 120), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 100, 0), 2);
        putText(img_to_show, "Time : " + timeStr() + " msec", Point(5, 150), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255, 100, 0), 2);
