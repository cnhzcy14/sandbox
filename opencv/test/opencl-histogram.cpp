...
private:
    ...
    cl_kernel m_kernelBufHistogram;
...
App::~App()
{
    ...
    if (m_kernelBufHistogram)
    {
        clReleaseKernel(m_kernelBufHistogram);
        m_kernelBufHistogram = 0;
    }
...
int App::initOpenCL()
{
    ...
    for (i = 0; i < m_platform_ids.size(); i++)
    {
        ...
        m_kernelBufHistogram = clCreateKernel(m_program, "histogram", &res);
        if (0 == m_kernelBufHistogram || CL_SUCCESS != res)
            return -1;
...
int App::process_frame_with_open_cl(cv::Mat &frame, bool use_buffer, cl_mem *mem_obj)
{
    ...
    const int histogramSize = 256 * sizeof(int);
    ...
    if (0 == mem)
    {
        ...
        if (use_buffer)
        {
            ...
            m_mem_hist = clCreateBuffer(m_context, CL_MEM_WRITE_ONLY, histogramSize, NULL, &res);
            if (0 == m_mem_hist || CL_SUCCESS != res)
                return -1;

            res = clSetKernelArg(m_kernelBufHistogram, 0, sizeof(cl_mem), &mem);
            if (CL_SUCCESS != res)
                return -1;

            const int imageElements = frame.rows * frame.cols;
            res = clSetKernelArg(m_kernelBufHistogram, 1, sizeof(int), &imageElements);
            if (CL_SUCCESS != res)
                return -1;

            res = clSetKernelArg(m_kernelBufHistogram, 2, sizeof(cl_mem), &m_mem_hist);
            if (CL_SUCCESS != res)
                return -1;

            kernel = m_kernelBufHistogram;
        }
        else
        {
    ...
    if (use_buffer)
    {
        ...
        int zero = 0;
        res |= clEnqueueFillBuffer(m_queue, m_mem_hist, &zero, sizeof(int), 0, histogramSize, 0, NULL, NULL);
        res |= clEnqueueNDRangeKernel(m_queue, m_kernelBufHistogram, 1, 0, globalWorkSize, 0, 0, 0, &asyncEvent);
        if (CL_SUCCESS != res)
            return -1;
    }
    else
