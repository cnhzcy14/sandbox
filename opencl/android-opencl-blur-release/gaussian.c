...
char pna_blur_cpu(char* imgname,uint32_t size,float sigma)
{
    ...
    free(matrix);
    printf("rk-debug cpu run ok write cpu_blur.bmp \n");
...
char pna_blur_gpu(char* imgname,uint32_t size,float sigma)
{
    ...
    if( (f = fopen("/home/cnhzcy14/work/project/test_code/opencl/android-opencl-blur-release/kernel.cl", "r")) == NULL)
    {
    ...
    if (ret < 0)
    {
        perror("Didn't finish");
        exit(1);
    }

    run_time = wtime() - start_time;
    printf("Real kernel time: %.2f ms \n", run_time * 1000);
    ...
    printf("rk-debug gpu run ok write gpu_blur.bmp \n");
    return true;
}
